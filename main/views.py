...
from .models import CustomUser,Plan,Line,Product,Transctions,WithdrawalRequest,Links
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                if referrer.plan.id == 1:
                    referrer.reward_earning += 150
                    referrer.save()
                elif referrer.plan.id ==2:
                    referrer.reward_earning += 275
                    referrer.save()
                elif referrer.plan.id == 3:
                    referrer.reward_earning += 580
                    referrer.save()
                elif referrer.plan.id == 4:
                    referrer.reward_earning += 1150
                    referrer.save()
                else:
                    referrer.Balance += 0
                    referrer.save()
...
class LinkClickView(LoginRequiredMixin, View):
    def get(self, request, link_id, *args, **kwargs):
        ...
        if not getattr(user, link_attr, False):
            ...
            user.Balance += user.plan.link_earning
            user.save()
        else:
